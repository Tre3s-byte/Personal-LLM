...
def _generate_with_model(model_name: str, messages: List[Dict[str, str]], max_tokens: int | None = None) -> str:
...
def _hierarchical_log_generate(model_name: str, messages: List[Dict[str, str]]) -> str:
    ...
    for i, chunk in enumerate(chunks, start=1):
        prompt_messages = [
            {"role": "system", "content": "Summarize technical logs. Keep root causes, errors, and timestamps."},
            {"role": "user", "content": f"Chunk {i}/{len(chunks)}\n\n{chunk}"},
        ]
        chunk_summaries.append(_generate_with_model(model_name, prompt_messages, max_tokens=350))

    merge_messages = [
        {"role": "system", "content": "Merge chunk summaries into one coherent report with key findings and action items."},
...
def _structured_code_generate(model_name: str, messages: List[Dict[str, str]]) -> str:
    ...
    for i, chunk in enumerate(code_chunks, start=1):
        prompt_messages = [
            {"role": "system", "content": "Summarize this code block: purpose, dependencies, and potential issues."},
            {"role": "user", "content": f"Code block {i}/{len(code_chunks)}\n\n{chunk}"},
        ]
        function_summaries.append(_generate_with_model(model_name, prompt_messages, max_tokens=300))
...
def run_routed_inference(model_name: str, messages: List[Dict[str, str]], routing: Dict[str, str]):
    ...
    if strategy == "chat":
        ...
        return run_inference(model_name, trimmed_messages)

    if strategy == "log":
        return _hierarchical_log_generate(model_name, messages)

    if strategy == "code":
        return _structured_code_generate(model_name, messages)
